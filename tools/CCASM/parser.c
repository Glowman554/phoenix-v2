...
void parse_id() {
    if (is_instruction()) {
        for (int i = 0; i < INSTRUCTIONS_LEN; i++) {
            if (strcmp(instructions[i], current_token.string_data) == 0) corresponding_function[i] (instruction_opcodes[i]);
        }
...
int parse_second_class(int expected_type) {
    if (current_token.type == ID && get_label(AST.labels, &AST.number_of_labels, current_token.string_data).value >= 0) {
        if (expected_type != IMM16_TYPE) throw_error("Type error", enable_errors);
        ...
        for (int i = 16; i < DREG_LEN+16; i++) {
            if (strcmp(d_registers[i-16], current_token.string_data) == 0) return d_register_opcodes[i];
        }
    } else if (current_token.type == NUMBER8) {
        if (expected_type != IMM8_TYPE) throw_error("Type error", enable_errors);
        return current_token.imm16_data;
    } else if (current_token.type == NUMBER16) {
        if (expected_type != IMM16_TYPE && expected_type != NUMBER8) throw_error("Type error", enable_errors);
        return current_token.imm16_data;
    } else {
        char err[36];
...
void parse_comma() {
    if (IS_COMMA()) p_advance();
    else throw_error("Comma expected!", enable_errors);
}

int get_opcode_increment_from_opcode(int x) {
    // switch to get all the I functions
    switch (x) {
        case INSTR_ADD: return INSTR_ADDI;break;
        case INSTR_CMP: return INSTR_CMPI;break;
        case INSTR_JEQA: return INSTR_JEQI;break;
        case INSTR_JMPA: return INSTR_JMPI;break;
        case INSTR_JNQA: return INSTR_JNQI;break;
        case INSTR_JNZA: return INSTR_JNZI;break;
        case INSTR_JZRA: return INSTR_JZRI;break;
        case INSTR_NAD: return INSTR_NADI;break;
        case INSTR_NOR: return INSTR_NORI;break;
        case INSTR_SUB: return INSTR_SUBI;break;
...
int get_opcode_increment_from_for_b(int x) {
    ...
    switch (x) {
        case INSTR_INPA: return INSTR_INPB;break;
        case INSTR_JEQA: return INSTR_JEQB;break;
        case INSTR_JMPA: return INSTR_JMPB;break;
        case INSTR_JNQA: return INSTR_JNQB;break;
        case INSTR_JNZA: return INSTR_JNZB;break;
        case INSTR_LADA: return INSTR_LADB;break;
        case INSTR_LDRA: return INSTR_LDRB;break;
        case INSTR_OUTA: return INSTR_OUTB;break;
        case INSTR_WTRA: return INSTR_WTRB;break;
        default: return x;break;
    }
    return x;
}
